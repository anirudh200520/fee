...
export const bookmark_my_job = async (req, res) => {
    ...
    try {
        const checkAlreadyBookMarked = await bookMarkJob.findOne({ where: { job_id: job, user_id: user } })
        if (checkAlreadyBookMarked) return res.status(401).json({ success: false, message: "This Job is Already in Bookmark" })

        const bookmarkingJob = await bookMarkJob.create({ job_id: job, user_id: user });
...
export const getBookmark_jobs = async (req, res) => {
    ...
    try {
        const getBookMark = await bookMarkJob.findAll({
...
export const delete_bookmark_job = async (req, res) => {
    ...
    try {
        ...
        const deleteBookmark = await bookMarkJob.destroy({ where: { id: id } })
